...
pub struct StagenetProxyType(pub ProxyType);

impl From<ProxyType> for StagenetProxyType {
    fn from(proxy_type: ProxyType) -> Self {
        Self(proxy_type)
    }
}

impl Default for StagenetProxyType {
    fn default() -> Self {
...
impl pallet_evm_precompile_proxy::EvmProxyCallFilter for StagenetProxyType {
...
impl frame_support::traits::InstanceFilter<RuntimeCall> for StagenetProxyType {
    fn filter(&self, c: &RuntimeCall) -> bool {
        self.0.filter(c)
    }
...
impl pallet_proxy::Config for Runtime {
    ...
    type RuntimeCall = RuntimeCall;
    type Currency = Balances;
    type ProxyType = StagenetProxyType;
