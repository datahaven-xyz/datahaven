...
pub struct TestnetProxyType(pub ProxyType);

impl From<ProxyType> for TestnetProxyType {
    fn from(proxy_type: ProxyType) -> Self {
        Self(proxy_type)
    }
}

impl Default for TestnetProxyType {
    fn default() -> Self {
...
impl pallet_evm_precompile_proxy::EvmProxyCallFilter for TestnetProxyType {
...
impl frame_support::traits::InstanceFilter<RuntimeCall> for TestnetProxyType {
    fn filter(&self, c: &RuntimeCall) -> bool {
        self.0.filter(c)
    }
...
impl pallet_proxy::Config for Runtime {
    ...
    type RuntimeCall = RuntimeCall;
    type Currency = Balances;
    type ProxyType = TestnetProxyType;
