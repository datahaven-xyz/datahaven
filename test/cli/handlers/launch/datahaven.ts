...
export async function prepareDatahavenAuthoritiesConfig(): Promise<void> {
	...
	for (const compressedKey of authoritiesToProcess) {
		try {
			...
			logger.error(
				`❌ Failed to process public key ${compressedKey}: ${error}`,
			);
	...
	try {
		...
		if (!configJson.snowbridge) {
			...
			logger.warn(
				`"snowbridge" section not found in ${configFilePath}, created it.`,
			);
		}
		...
		logger.success(
			`✅ Datahaven validator hashes updated in: ${configFilePath}`,
		);
	} catch (error) {
...
const shouldRebuildBinary = async (binaryPath: string): Promise<boolean> => {
	...
	try {
		...
		const gitStatusProcess =
			await $`cd ${operatorPath} && git status --porcelain`.quiet();
		...
		logger.info(
			"✅ No changes detected in the operator folder, skipping build",
		);
		return false;
	} catch (error) {
		logger.warn(
			`⚠️ Error checking Git status/diff: ${error}. Will rebuild to be safe.`,
		);
		return true;
	}
};
...
export const performDatahavenOperations = async (
	...
	for (const id of CLI_AUTHORITY_IDS) {
		...
		invariant(
			completed,
			"❌ Could not find 'Running JSON-RPC server:' in logs",
		);
