...
export const setupValidators = async (
	options: SetupValidatorsOptions,
): Promise<boolean> => {
	const {
		rpcUrl,
		validatorsConfig,
	...
	const configPath =
		validatorsConfig ||
		path.resolve(__dirname, "../configs/validator-set.json");
	...
	if (
		...
		logger.error(
			"Invalid validator configuration: 'validators' array is missing or empty",
		);
		throw new Error(
			"Validator configuration must contain a non-empty 'validators' array",
		);
	}
	...
	for (const [index, validator] of config.validators.entries()) {
		...
		if (!validator.publicKey.startsWith("0x")) {
			throw new Error(
				`Validator publicKey at index ${index} must start with '0x'`,
			);
		}
		if (!validator.privateKey.startsWith("0x")) {
			throw new Error(
				`Validator privateKey at index ${index} must start with '0x'`,
			);
		}
	}
	...
	for (let i = 0; i < validators.length; i++) {
		...
		await runShellCommandWithLogger(signupCommand, {
			env,
			cwd: "../contracts",
		});
